...
class BadRequestError extends TaggedError("BadRequestError") {
    getMessage() {
...
const fetchEff = (input, init)=>Effect.flatMap(FetchContext, ({ fetch, baseHeaders })=>{
        const reqInfo = {
            url: input.toString(),
            method: init?.method,
            body: init?.body,
        ...
        }).pipe(Effect.map((res)=>Object.assign(res, {
...
const parseResponseJson = (res)=>Effect.tryPromise({
        try: async ()=>{
    ...
    }).pipe(Effect.filterOrFail(({ ok })=>ok, ({ json, status })=>new BadRequestError({
            status,
            message: `Request to ${res.requestUrl} failed with status ${status}`,
            json
        })), Effect.map(({ json })=>json), Effect.withSpan("parseJson"));
const parseRequestJson = (req)=>Effect.tryPromise({
        try: ()=>req.json(),
        catch: (error)=>new InvalidJsonError({
                error,
                input: req.url
...
export { ALLOWED_FILE_TYPES, BadRequestError, FILESIZE_UNITS, FetchContext, FetchError, INTERNAL_DO_NOT_USE__fatalClientError, INTERNAL_doFormatting, InvalidFileSizeError, InvalidFileTypeError, InvalidJsonError, InvalidRouteConfigError, InvalidURLError, RetryError, UnknownFileTypeError, UploadThingError, ValidACLs, ValidContentDispositions, allowedContentTextLabelGenerator, asArray, bytesToFileSize, capitalizeStart, contentDisposition, contentFieldToContent, exponentialBackoff, fetchEff, fileSizeToBytes, fillInputRouteConfig, filterObjectValues, generateClientDropzoneAccept, generateMimeTypes, generatePermittedFileTypes, generateUploadThingURL, getDefaultSizeForType, getErrorTypeFromStatusCode, getFilesFromClipboardEvent, getFullApiUrl, getRequestUrl, getStatusCodeFromError, getTypeFromFileName, isObject, isRouteArray, objectKeys, parseRequestJson, parseResponseJson, resolveMaybeUrlArg, safeParseJSON, semverLite, signPayload, styleFieldToClassName, styleFieldToCssObject, verifySignature };
