...
const UploadedFileDataSchema = S__namespace.extend(FileUploadDataWithCustomIdSchema, S__namespace.Struct({
...
const PollUploadResponseSchema = S__namespace.Struct({
...
const FailureCallbackResponseSchema = S__namespace.Struct({
    success: S__namespace.Boolean,
    message: S__namespace.optional(S__namespace.String)
});
const ServerCallbackPostResponseSchema = S__namespace.Struct({
    status: S__namespace.String
});
/**
 * =============================================================================
 ...
 * =============================================================================
 */ const UploadActionPayload = S__namespace.Struct({
    files: S__namespace.Array(FileUploadDataSchema),
    input: S__namespace.Unknown
});
const FailureActionPayload = S__namespace.Struct({
...
const MultipartCompleteActionPayload = S__namespace.Struct({
    fileKey: S__namespace.String,
    uploadId: S__namespace.String,
...
const conditionalDevServer = (fileKey, apiKey)=>{
    return Effect__namespace.gen(function*() {
        const file = yield* shared.fetchEff(shared.generateUploadThingURL(`/api/pollUpload/${fileKey}`)).pipe(Effect__namespace.andThen(shared.parseResponseJson), Effect__namespace.andThen(S__namespace.decodeUnknown(PollUploadResponseSchema)), Effect__namespace.andThen((res)=>res.status === "done" ? Effect__namespace.succeed(res.fileData) : Effect__namespace.fail(new shared.RetryError())), Effect__namespace.retry({
 ...
 */ const uploadPart = (opts)=>shared.fetchEff(opts.url, {
        ...
        abortMultipartUpload({
            ...
            Effect__namespace.fail(new shared.UploadThingError({
                code: "UPLOAD_FAILED",
...
const abortMultipartUpload = (presigned)=>shared.fetchEff(shared.generateUploadThingURL("/api/failureCallback"), {
    ...
    }).pipe(Effect__namespace.andThen(shared.parseResponseJson), Effect__namespace.andThen(S__namespace.decodeUnknown(FailureCallbackResponseSchema)), Effect__namespace.withSpan("abortMultipartUpload", {
        attributes: {
            presigned
        }
    }));
...
class FileSizeMismatch extends Data.TaggedError("FileSizeMismatch") {
...
class FileCountMismatch extends Data.TaggedError("FileCountMismatch") {
...
class RequestInput extends Context__namespace.Tag("uploadthing/RequestInput")() {
}
...
const handleCallbackRequest = Effect__namespace.gen(function*() {
    ...
    }).pipe(Effect__namespace.andThen(shared.parseResponseJson), Effect__namespace.andThen(S__namespace.decodeUnknown(ServerCallbackPostResponseSchema)));
    return {
        body: null
    };
});
...
const handleUploadAction = Effect__namespace.gen(function*() {
    ...
    }).pipe(Effect__namespace.andThen(shared.parseResponseJson), Effect__namespace.andThen(S__namespace.decodeUnknown(PresignedURLResponseSchema)));
...
const uploadPresignedPost = (file, presigned)=>Effect__namespace.gen(function*() {
        ...
        const res = yield* shared.fetchEff(presigned.url, {
        ...
        }).pipe(Effect__namespace.tapErrorCause(()=>shared.fetchEff(shared.generateUploadThingURL("/api/failureCallback"), {
                method: "POST",
                body: JSON.stringify({
                    fileKey: presigned.key,
                    uploadId: null
            ...
            }).pipe(Effect__namespace.andThen(shared.parseResponseJson), Effect__namespace.andThen(S__namespace.decodeUnknown(FailureCallbackResponseSchema)))));
        if (!res.ok) {
...
const uploadFilesInternal = (input)=>getPresignedUrls(input).pipe(Effect__namespace.andThen((presigneds)=>Effect__namespace.forEach(presigneds, (file)=>uploadFile(file).pipe(Effect__namespace.match({
                onFailure: (error)=>({
                        data: null,
...
const getPresignedUrls = (input)=>Effect__namespace.gen(function*() {
        ...
        const presigneds = yield* shared.fetchEff(shared.generateUploadThingURL("/api/uploadFiles"), {
            method: "POST",
            cache: "no-store",
            body: JSON.stringify({
                files: fileData,
...
const uploadFile = (input)=>Effect__namespace.gen(function*() {
        ...
        yield* shared.fetchEff(shared.generateUploadThingURL(`/api/pollUpload/${presigned.key}`)).pipe(Effect__namespace.andThen(shared.parseResponseJson), Effect__namespace.andThen(S__namespace.decodeUnknown(PollUploadResponseSchema)), Effect__namespace.tap(Effect__namespace.logDebug("Polled upload", presigned.key)), Effect__namespace.andThen((res)=>res.status === "done" ? Effect__namespace.succeed(undefined) : Effect__namespace.fail(new shared.RetryError())), Effect__namespace.retry({
            while: (err)=>err instanceof shared.RetryError,
            schedule: shared.exponentialBackoff
        }), Effect__namespace.catchTag("RetryError", (e)=>Effect__namespace.die(e)));
        return {
    ...
    constructor(opts){
        this.requestUploadThing = (pathname, body, responseSchema)=>{
            ...
            return shared.fetchEff(url, {
                ...
                headers: {
                    ...shared.filterObjectValues(this.defaultHeaders, (v)=>typeof v === "string"),
                    "Content-Type": "application/json"
                }
        ...
        this.executeAsync = (program)=>program.pipe(withMinimalLogLevel(this.logLevel), Effect__namespace.provide(ConsolaLogger), Effect__namespace.provideService(shared.FetchContext, {
                fetch: this.fetch,
   ...
   */ this.listFiles = async (opts)=>{
        ...
        this.getUsageInfo = async ()=>{
            ...
            const responseSchema = S__namespace.Struct({
                ...
                totalReadable: S__namespace.String,
                appTotalBytes: S__namespace.Number,
                appTotalReadable: S__namespace.String,
                filesUploaded: S__namespace.Number,
                limitBytes: S__namespace.Number,
                limitReadable: S__namespace.String
            });
