import { ValidContentDispositions, ValidACLs, fetchEff, generateUploadThingURL, parseResponseJson, RetryError, exponentialBackoff, UploadThingError, signPayload, isObject, contentDisposition, bytesToFileSize, getTypeFromFileName, objectKeys, InvalidRouteConfigError, fileSizeToBytes, FetchContext, getFullApiUrl, verifySignature, parseRequestJson, fillInputRouteConfig, filterObjectValues, asArray, getStatusCodeFromError } from '@uploadthing/shared';
...
const UploadedFileDataSchema = S.extend(FileUploadDataWithCustomIdSchema, S.Struct({
    key: S.String,
    url: S.String
}));
/**
...
const PollUploadResponseSchema = S.Struct({
    status: S.String,
...
const FailureCallbackResponseSchema = S.Struct({
    success: S.Boolean,
    message: S.optional(S.String)
});
const ServerCallbackPostResponseSchema = S.Struct({
    status: S.String
});
/**
 * =============================================================================
 ...
 * =============================================================================
 */ const UploadActionPayload = S.Struct({
    files: S.Array(FileUploadDataSchema),
    input: S.Unknown
});
const FailureActionPayload = S.Struct({
    fileKey: S.String,
    uploadId: S.NullOr(S.String),
    storageProviderError: S.optional(S.String),
    fileName: S.String
});
const MultipartCompleteActionPayload = S.Struct({
    fileKey: S.String,
    uploadId: S.String,
...
const conditionalDevServer = (fileKey, apiKey)=>{
    return Effect.gen(function*() {
        const file = yield* fetchEff(generateUploadThingURL(`/api/pollUpload/${fileKey}`)).pipe(Effect.andThen(parseResponseJson), Effect.andThen(S.decodeUnknown(PollUploadResponseSchema)), Effect.andThen((res)=>res.status === "done" ? Effect.succeed(res.fileData) : Effect.fail(new RetryError())), Effect.retry({
 ...
 */ const uploadPart = (opts)=>fetchEff(opts.url, {
        ...
        abortMultipartUpload({
            ...
            Effect.fail(new UploadThingError({
                code: "UPLOAD_FAILED",
...
const abortMultipartUpload = (presigned)=>fetchEff(generateUploadThingURL("/api/failureCallback"), {
    ...
    }).pipe(Effect.andThen(parseResponseJson), Effect.andThen(S.decodeUnknown(FailureCallbackResponseSchema)), Effect.withSpan("abortMultipartUpload", {
        attributes: {
            presigned
        }
    }));
...
class RequestInput extends Context.Tag("uploadthing/RequestInput")() {
}
...
const handleCallbackRequest = Effect.gen(function*() {
    ...
    }).pipe(Effect.andThen(parseResponseJson), Effect.andThen(S.decodeUnknown(ServerCallbackPostResponseSchema)));
    return {
        body: null
    };
});
...
const handleUploadAction = Effect.gen(function*() {
    ...
    }).pipe(Effect.andThen(parseResponseJson), Effect.andThen(S.decodeUnknown(PresignedURLResponseSchema)));
...
const uploadPresignedPost = (file, presigned)=>Effect.gen(function*() {
        ...
        const res = yield* fetchEff(presigned.url, {
        ...
        }).pipe(Effect.tapErrorCause(()=>fetchEff(generateUploadThingURL("/api/failureCallback"), {
                method: "POST",
                body: JSON.stringify({
                    fileKey: presigned.key,
                    uploadId: null
            ...
            }).pipe(Effect.andThen(parseResponseJson), Effect.andThen(S.decodeUnknown(FailureCallbackResponseSchema)))));
        if (!res.ok) {
...
const uploadFilesInternal = (input)=>getPresignedUrls(input).pipe(Effect.andThen((presigneds)=>Effect.forEach(presigneds, (file)=>uploadFile(file).pipe(Effect.match({
                onFailure: (error)=>({
                        data: null,
...
const getPresignedUrls = (input)=>Effect.gen(function*() {
        ...
        const presigneds = yield* fetchEff(generateUploadThingURL("/api/uploadFiles"), {
            method: "POST",
            cache: "no-store",
            body: JSON.stringify({
                files: fileData,
...
const uploadFile = (input)=>Effect.gen(function*() {
        ...
        yield* fetchEff(generateUploadThingURL(`/api/pollUpload/${presigned.key}`)).pipe(Effect.andThen(parseResponseJson), Effect.andThen(S.decodeUnknown(PollUploadResponseSchema)), Effect.tap(Effect.logDebug("Polled upload", presigned.key)), Effect.andThen((res)=>res.status === "done" ? Effect.succeed(undefined) : Effect.fail(new RetryError())), Effect.retry({
            while: (err)=>err instanceof RetryError,
            schedule: exponentialBackoff
        }), Effect.catchTag("RetryError", (e)=>Effect.die(e)));
        return {
    ...
    constructor(opts){
        this.requestUploadThing = (pathname, body, responseSchema)=>{
            ...
            return fetchEff(url, {
                ...
                headers: {
                    ...filterObjectValues(this.defaultHeaders, (v)=>typeof v === "string"),
                    "Content-Type": "application/json"
                }
        ...
        this.executeAsync = (program)=>program.pipe(withMinimalLogLevel(this.logLevel), Effect.provide(ConsolaLogger), Effect.provideService(FetchContext, {
                fetch: this.fetch,
